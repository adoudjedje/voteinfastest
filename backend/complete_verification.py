...
def get_existing_students():
    ...
    for etudiant in etudiants:
        ...
        existing[numero] = data
    return existing

def parse_all_students_from_pdf(text):
    """Parser tous les étudiants du PDF"""
    all_students = []
    lines = text.split('\n')
    
    print("🔍 Parsing de tous les étudiants du PDF...")
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        patterns = [
            # Pattern 1: numéro + matricule + PGPA + nom + téléphone
            r'^(\d{1,3})\s+(\d{2}[P-]?\d{4,5})PGPA([A-Z\s]+?)\s+(\d{10,11}(?:/\d{10,11})?)$',
            # Pattern 2: numéro + matricule + PGPA + nom + téléphone (avec espaces)
            r'^(\d{1,3})\s+(\d{2}[P-]?\d{4,5})\s+PGPA\s+([A-Z\s]+?)\s+(\d{10,11}(?:/\d{10,11})?)$',
            # Pattern 3: numéro + matricule + PGPA + nom + téléphone (format compact)
...
def find_missing_students(pdf_students, existing_students):
    """Trouver les étudiants manquants"""
    missing = []
    existing_numbers = set(existing_students.keys())
    
    print(f"📊 Étudiants existants: {len(existing_numbers)}")
    print(f"📄 Étudiants dans le PDF: {len(pdf_students)}")
...
def main():
    print("🔍 VÉRIFICATION COMPLÈTE DE LA LISTE DES ÉTUDIANTS")
    print("=" * 60)
    ...
    # Parser tous les étudiants du PDF
    pdf_students = parse_all_students_from_pdf(text)
    ...
    # Trouver les étudiants manquants
    missing_students = find_missing_students(pdf_students, existing_students)
